...
# ===== AI GEMINI doc captcha (FREE ~1000-1500 luot/ngay) =====
# Lay API key free tai: https://aistudio.google.com/apikey
# Dan key vao file "gemini_key.txt" (cung thu muc) HOAC dien thang vao GEMINI_KEY duoi.
GEMINI_KEY = ""
GEMINI_MODEL = "gemini-2.5-flash"   # doc anh tot; neu het quota doi "gemini-2.5-flash-lite"
try:
    if os.path.exists("gemini_key.txt"):
        _gk = open("gemini_key.txt", encoding="utf-8").read().strip()
        if _gk: GEMINI_KEY = _gk
except: pass

def read_captcha_gemini(data):
    """Gui anh captcha cho Gemini AI -> tra ve chuoi ky tu. '' neu loi/khong co key."""
    if not GEMINI_KEY or not data: return ""
    try:
        ...
        body = {
            "contents": [{"parts": [
                {"text": "Read the captcha in this image. It contains ONLY letters and digits. "
                         "Reply with ONLY those characters, uppercase, no spaces, no quotes, no explanation."},
        ...
        print("   [!] Gemini lỗi:", str(e)[:100]); return ""
...
if GEMINI_KEY: AUTO_CAPTCHA = True   # co AI Gemini -> chay duoc ke ca khi thieu ddddocr
...
def read_captcha_text(driver, img):
    """UU TIEN AI Gemini (chinh xac cao ~90%); khong co key/loi -> fallback ddddocr.
       Tra (text, strong)."""
    data = get_captcha_bytes(img)
    if not data:
        return "", False
    # ===== 1) AI GEMINI doc truoc (neu co key) =====
    if GEMINI_KEY:
        gt = read_captcha_gemini(data)
        if 4 <= len(gt) <= 8:
            print(f"   [AI] Gemini đọc: {gt}")
            return gt, True          # AI doc -> tin cao -> gui luon
        else:
            print("   [AI] Gemini không đọc được -> dùng ddddocr")
    # ===== 2) Fallback ddddocr =====
...
def main():
    print("Apple Checker v6.0-AI (check1) - Gemini AI doc captcha (~90%) + fallback ddddocr")
