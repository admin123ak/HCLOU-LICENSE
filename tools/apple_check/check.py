# Apple Serial Checker - Bản Vá Chặn Đứng Vòng Lặp Tự Reset Của Apple
...
# ===== CẤU HÌNH PROXY =====
PROXY_KEY = ""        
PROXY_NHAMANG = "Random"  # Đổi thành "Viettel" hoặc "Vina" nếu dải FPT bị chậm/lag
PROXY_TINH = "0"          
PROXY_API_URL = ""        
PROXY_MIN_SEC = 62        
PROXY = ""                
...
UNATTENDED = True
CAPTCHA_TRIES = 5 # Giảm số lần thử trên 1 IP xuống để an toàn, dính lỗi là đổi IP ngay
...
def set_anti_sleep(status=True):
    try:
        ...
        if status:
            ...
            print("   [+] Đã kích hoạt chế độ CHỐNG NGỦ. Treo máy xuyên đêm mượt mà!")
        else:
...
def load_existing_results(fp=RESULT_FILE):
    ...
    try:
        ...
        print("   [!] Chưa có file kết quả cũ, sẽ ghi mới.")
    return out
...
def save_results(master_serials, existing_results, fp=RESULT_FILE):
    ...
    try: wb.save(fp)
    except PermissionError: print("   !! [!] CẢNH BÁO: Hãy ĐÓNG file Excel kết quả ngay để Tool ghi dữ liệu!")
...
def find_continue(driver):
    return _find(driver, [
        ...
        (By.XPATH, "//button[contains(text(), 'Gửi') or contains(text(), 'Submit') or contains(text(), 'Continue')]"),
        (By.ID, "captcha-action"),
    ])

def force_refresh_captcha(driver):
    el = _find(driver, [
        (By.CSS_SELECTOR, "[aria-label*='refresh' i],[aria-label*='lam moi' i]"),
        (By.CSS_SELECTOR, ".captcha-container button, .captcha-container a"),
    ])
    if el:
        try: 
...
def submit_and_check(driver, timeout=5):
    ...
    # Hãm phanh tĩnh: Chờ kết quả phản hồi của Apple, KHÔNG bấm loạn xạ
...
def solve_captcha_loop(driver, serial):
    ...
    while attempt < CAPTCHA_TRIES:
        ...
        if not img or not cin:
            if looks_banned(driver): return "BANNED"
            blank_counter += 1
            if blank_counter > 4: return "TIMEOUT"
        ...
        # FIX: Né hoàn toàn việc gửi mã lỗi độ dài (Dưới 4 ký tự) làm rách Token Apple
        if not text or len(text) != 4:
            print(f"   [!] Mã lỗi định dạng ({text or 'Trống'}). Đang đổi ảnh mới...")
            force_refresh_captcha(driver)
            time.sleep(1.5)
            continue
        ...
        # GỬI VÀ THEO DÕI BIẾN ĐỘNG TRANG
        status = submit_and_check(driver, timeout=5)
        if status == "success": return True 
        if status == "BANNED": return "BANNED"
        
        # THIẾT LẬP CHỐNG SPAM: Nếu Apple tự load lại hình mới (Do Token chết) hoặc báo sai mã
        print("   [!] Hệ thống phản hồi / Từ chối Token. Tiến hành cập nhật trạng thái...")
        attempt += 1
        time.sleep(1.5)
        
        # Đồng bộ lại ô nhập Serial đề phòng Apple xóa trống
        s_box = find_serial_input(driver, timeout=2)
        if s_box:
            cv = s_box.get_attribute("value") or ""
            if cv != serial: smooth_type(driver, s_box, serial)
...
def handle(driver, serial, i, total):
    try:
        ...
        try: driver.get("https://checkcoverage.apple.com/?locale=vi_VN")
        except:
...
def main():
    print("Apple Checker v3.0 - Hệ Thống Chống Reset Token")
    ...
    if cur_proxy:
        print("   [+] Chờ 10 giây cho cổng proxy khởi động mượt mà...")
        time.sleep(10)
        ...
        while idx < len(serials_to_check):
            ...
            try:
                if done_on_ip >= BATCH_PER_IP:
                    ...
                    print("   [+] Chờ 12 giây cho IP mới ổn định kết nối...")
                    time.sleep(12)
                    d = create_browser(cur_proxy); done_on_ip = 0
                ...
                if r.status in ("BANNED", "RETRY", "TIMEOUT", "ERROR"):
                    ...
                    print(f"   [!] Gặp lỗi ({r.status}). Đóng trình duyệt, gọi IP mới khẩn cấp...")
                    try: d.quit()
                    except: pass
                    if (PROXY_API_URL or PROXY_KEY):
                        new = fetch_rotating_proxy()
                        if new: cur_proxy = new
                    print("   [+] Hãm phanh an toàn: Nghỉ 12 giây ngăn chặn Apple truy vết...")
                    time.sleep(12)
                    d = create_browser(cur_proxy); done_on_ip = 0
                    continue   
