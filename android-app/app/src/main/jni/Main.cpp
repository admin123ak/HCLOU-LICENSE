...
// HCLOU-LICENSE master secrets — embed plain, XOR encrypt trước release
...
Java_com_android_support_TechnicalAkash1_Check(JNIEnv *env, jclass clazz, jobject mContext, jstring mUserKey) {
    ...
    if (curl) {
        ...
        // HCLOU-LICENSE: build HMAC sign + nonce + timestamp
        ...
        if (res == CURLE_OK) {
            try {
                ...
                if (result[/*status*/ StrEnc("(>_LBm", "\x5B\x4A\x3E\x38\x37\x1E", 6).c_str()] == true) {
                    ...
                    if (rng + 30 > time(0)) {
                        // HCLOU-LICENSE: derive per_static thay vì STATIC_WORD trực tiếp
