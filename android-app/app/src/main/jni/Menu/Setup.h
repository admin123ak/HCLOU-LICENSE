...
void Init(JNIEnv *env, jobject thiz, jobject ctx, jobject title, jobject subtitle){
    //Set sub title
    setText(env, title, OBFUSCATE("<b> QSG MODZ VIP </b>"));

    //Set sub title
    setText(env, subtitle, OBFUSCATE("<b><marquee><p style=\"font-size:30\">"
                                     "<p style=\"color:green;\">BẢN QUYỀN THUỘC QSG MODZ</p> "
    ...
    //Toast Example
    Toast(env,ctx,OBFUSCATE("QSG MODZ MÃI ĐỈNH NHÉ:))"),ToastLength::LENGTH_LONG);
